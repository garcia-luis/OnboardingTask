...
calculateMissing <- function(df, patientsCohort){
...
plotUniqueValues <- function(df, nameOfPlot = NULL){
  ...
  ##Output: A ggplot2 barplot
  	...
  	p <-ggplot(data=df, aes(x=variableName, y=percentage)) + geom_bar(stat="identity") #+ theme(axis.text.x = element_text(angle = 0))
	p <- p + labs(title="Data Availability", x ="", y = "% of Patients with Data")
	p <- p + theme(plot.title = element_text(hjust = 0.5)) + coord_flip()
  	if(length(nameOfPlot) == 0){nameOfPlot = "barplotUnique.png"} else {nameOfPlot = paste0(nameOfPlot, ".png")}
...
plotLongitudinalValues <- function(df, nameOfPlot = NULL){
  ...
  ##Output: A ggplot2 barplot
  	...
  	p <-ggplot(data=df, aes(x=variable_name, y=percentage_available)) + geom_bar(stat="identity") #+ theme(axis.text.x = element_text(angle = 90))
	p <- p + labs(title="Data Availability", x ="", y = "% of Patients with Data")
	p <- p + theme(plot.title = element_text(hjust = 0.5)) + coord_flip()
...
plotVisits <- function(df, nameOfPlot = "VisitsDistribution"){
  ...
  ##Output: A ggplot2 barplot
  	...
  	p <-ggplot(data=df, aes(x=concept_name, y=n)) + geom_bar(stat="identity") #+ theme(axis.text.x = element_text(angle = 90))
	p <- p + labs(title="Visits type Availability", x ="", y = "% of ocurrences")
	p <- p + theme(plot.title = element_text(hjust = 0.5)) + coord_flip()
  	if(length(nameOfPlot) == 0){nameOfPlot = "barplotlongitudinal.png"} else {nameOfPlot = paste0(nameOfPlot, ".png")}
